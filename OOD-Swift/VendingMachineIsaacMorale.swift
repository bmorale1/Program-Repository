...
class Product {
    ...
    func isEqual(_ object: Product?) -> Bool {
        return self.name == (object!.name) && self.value == object!.value
    }
...
class VendingMachine {
    ...
    func insertLabel(label: String) {
        if trays[label] != nil{
            if trays[label]!.objects[0].value >= insertedCoins.grandTotalValue(){
                dispenser.add(object:trays[label]!.objects.remove(at:1))
            }
            else{
                print("insufficient funds, please add additional coins")
            }
        } 
        else{
            print("no product available at given label")
        }
    }
}
